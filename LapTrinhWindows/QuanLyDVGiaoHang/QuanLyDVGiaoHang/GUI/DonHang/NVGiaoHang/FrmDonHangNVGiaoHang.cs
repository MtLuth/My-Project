...
{
    public partial class FrmDonHangNVGiaoHang : Form
    {
        private string TenDangNhap;
        public FrmDonHangNVGiaoHang(string tenDangNhap)
        {
            InitializeComponent();
        ...
        private void loadData()
        {
            using (var context = new QuanLyDVGiaoHangEntities())
            {
                List<DONHANG> donHangList = context.DONHANGs
                    .Where(dh => dh.TinhTrang == "Đang đợi nhân viên giao hàng tiếp nhận đơn hàng")
                    .ToList();
        ...
        {
            DialogResult result = MessageBox.Show("Bạn có muốn thoát bảng Đơn Hàng NV Giao Hàng ?", "Thông Báo",
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            using (var context = new QuanLyDVGiaoHangEntities())
            {
                var result = context.DONHANGs
                    .Where(dh => dh.TinhTrang == "Đang đợi nhân viên giao hàng tiếp nhận đơn hàng" && dh.MaVanDon.ToString().Contains(giatri))
                    .Select(dh => new
                    {
                        dh.MaVanDon,
                        dh.NguoiGui,
                ...
                using (var context = new QuanLyDVGiaoHangEntities())
                {
                    ...
                    if (donhang != null)
                    {
                        DS_DONHANGCANGIAONHAN dsDonHangCanGiaoNhan = new DS_DONHANGCANGIAONHAN()
                        {
                            ...
                            GiaoNhan = true,
                            TrangThai = "Đã Nhận",
                            GhiChu = "Nhân viên giao hàng đang trên đường giao đến bạn",
                        };
                        ...
                        MessageBox.Show("Đơn hàng đã được tiếp nhận", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    }
                }
            }
            else
